...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_classes', type=int, default=100, metavar='N',
    ...
    train_data, test_data = prepareDataset("/home/dejan_stepec/awe_capsnet/CapsNet-pytorch/dataset/awe/")
    ...
    def train(epoch):
        model.train()
        for batch_idx, (data, target) in enumerate(awe_train_loader):
            if args.cuda:
            ...
            if batch_idx % args.log_interval == 0:
                print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                    epoch, batch_idx * len(data), len(awe_train_loader.dataset),
                           100. * batch_idx / len(awe_train_loader), loss.data[0]))

    def test():
        model.eval()
        test_loss = 0
        correct = 0
        for data, target in awe_test_loader:
            if args.cuda:
                data, target = data.cuda(), target.cuda()
        ...
        test_loss /= len(awe_test_loader.dataset)
        print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
            test_loss, correct, len(awe_test_loader.dataset),
            100. * correct / len(awe_test_loader.dataset)))
        return test_loss
